...
def get_value(args, task, x, y, n_evaluate_sample, cache_value=True, api_key=None, api_base=None, model=None, gpt=None):
...
def get_values(args, task, x, ys, n_evaluate_sample, cache_value=True, api_key=None, api_base=None, model=None, gpt=None):
    values = []
    local_value_cache = {}
...
def get_votes(args, task, x, ys, n_evaluate_sample, api_key=None, api_base=None, model=None, gpt=None):
    vote_prompt = task.vote_prompt_wrap(x, ys)
    if gpt is None:
        vote_outputs = gpt(args, vote_prompt, n=n_evaluate_sample, stop=None, api_key=api_key, api_base=api_base, model=model, gpt=gpt)  # type: ignore
    else:
...
def get_proposals(args, step, task, x, y, api_key=None, api_base=None, model=None, gpt=None):
    # jinyu:
...
def get_samples(args, task, x, y, n_generate_sample, prompt_sample, stop, api_key=None, api_base=None, model=None, gpt=None):
...
class ToTMethods:
    ...
    def naive_solve(self, task, idx, to_print=True, solve_client="remote_client", **kwargs) -> Tuple[List[str], Dict[str, List[Dict[str, str]]]]:
    ...
    def solve(self, task, idx, to_print=True, solve_client="remote_client", **kwargs) -> Tuple[List[str], Dict[str, List[Dict[str, str]]]]:
        gpt = self.gpts[solve_client]
        print(gpt)
        x = task.get_input(idx)  # input
        ys = [""]  # current output candidates
        ...
        for step in range(task.steps):
            ...
            if self.args.method_generate == "sample":
                new_ys = [
                    get_samples(
                        self.args, task, x, y, self.args.n_generate_sample, prompt_sample=self.args.prompt_sample, stop=task.stops[step], gpt=gpt
                    )
                    for y in ys
                ]
            elif self.args.method_generate == "propose":
                new_ys = [get_proposals(self.args, step, task, x, y, gpt) for y in ys]
            new_ys = list(itertools.chain(*new_ys))
            ids = list(range(len(new_ys)))
            # evaluation
            if self.args.method_evaluate == "vote":
                values = get_votes(self.args, task, x, new_ys, self.args.n_evaluate_sample, gpt=gpt)
            elif self.args.method_evaluate == "value":
                values = get_values(self.args, task, x, new_ys, self.args.n_evaluate_sample, gpt=gpt)
    ...
    def speculative_solve(self, task, idx, to_print=True, solve_client=None, eval_client=None, **kwargs) -> Tuple[List[str], Dict[str, List[Dict[str, str]]]]:
        # Output the imformation of models called
        ...
        for step in range(task.steps):
            ...
            if self.args.method_generate == "sample":  # large model for sample
                ...
                new_ys = [
                    get_proposals(self.args, step, task, x, y, gpt=solve_client)
                    for y in ys
                ]
            else:
                raise Exception("Not match!")
            ...
            if self.args.method_evaluate == "vote":
                values = get_votes(
                    self.args, task, x, new_ys, self.args.n_evaluate_sample, gpt=eval_client
                )
            elif self.args.method_evaluate == "value":
                values = get_values(
                    self.args, task, x, new_ys, self.args.n_evaluate_sample, gpt=eval_client
    ...
    def federated_solve(self, task, idx, to_print=True, assign_function=assign_task, **kwargs) -> Tuple[List[str], Dict[str, List[Dict[str, str]]]]:
